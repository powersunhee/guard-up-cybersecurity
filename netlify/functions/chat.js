    ...
    if (!process.env.OPENAI_API_KEY) {
      return {
        ...
        body: JSON.stringify({ 
          ...
          fallback: true,
          message: "I'm running in local mode. For full AI features, the API key needs to be configured. I can still help with cybersecurity advice using my built-in knowledge!"
        }),
      };
    }
...
Keep responses concise but informative. Your goal is to help users become more security-aware.`;
    ...
    const completion = await openai.chat.completions.create({
      model: 'gpt-3.5-turbo', // Use gpt-4 if you want higher quality responses
      messages: messages,
      max_tokens: 500,
      temperature: 0.7,
    ...
    if (error.code === 'insufficient_quota') {
      ...
      fallbackResponse = "I'm receiving too many requests right now. Please try again in a few seconds, or I can help with general security questions using my built-in responses!";
    }
